...
test('local city is added', async ({ page }) => {
  ...
  await expect(page.getByLabel('City').nth(0)).toHaveValue('Makassar');
});
...
test('time changes from local time', async ({ page }) => {
  ...
  await page.getByLabel('Time').nth(0).fill('04/09/2025 01:00 PM');

  await expect(page.locator('xif-time-map')).toContainText('9 April 2025 at 05:00 UTC');

  await expect(page.locator('xif-time-map').first()).toHaveScreenshot();
});
...
test('time changes from specific city', async ({ page }) => {
  ...
  await page.getByLabel('Time').nth(londonIndex).fill('04/09/2025 06:00 AM');

  await expect(page.locator('xif-time-map')).toContainText('9 April 2025 at 05:00 UTC');

  // During winter, London is UTC+0
  await page.getByLabel('Time').nth(londonIndex).fill('01/09/2025 06:00 AM');
